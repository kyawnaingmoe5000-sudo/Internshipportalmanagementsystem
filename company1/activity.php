				...
				<div  class = " panel-heading">	
					<table id = "table" class = "table table-bordered">
						...
						<tbody>
							...
							$stmt->fetch();
							$stmt->close();
							$act_query = $conn->query("SELECT * FROM `activity` WHERE `company_name` = '$name'") or die($conn->error);
					...
					<div style = "width:40%; margin-left:32%;">	
						<form method = "POST" action = "add_activity.php">	
							...
							<div class = "form-group">
								...
								<input type = "text" class = "form-control" name = "company_name" value="<?php echo $name?>" required = "required" disabled/>
							</div>
							<div class = "form-group">
								<label>Available</label>
								<input type = "number" class = "form-control" name = "quantity" max="20" required = "required"/>
							</div>
