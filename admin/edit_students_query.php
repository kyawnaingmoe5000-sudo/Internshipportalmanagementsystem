<?php
	...
	if(ISSET($_POST['update_admin'])){
		...
		if($v_admin > 0){
			...
			$conn->query("UPDATE `registered_users` SET `user_name` = '$username', `first_name` = '$firstname', last_name=' $lastname ',academic_year=' $academicyear ', `password` = '$password', email=' $email' ,gender='$gender' WHERE `id` = '$_REQUEST[id]'") or die($conn->error);
