<div id = "sidebar">
	<ul id = "menu" class = "nav menu">
		<li>
			<a>
				<?php 
					require_once '../connect.php';
					$q_admin_side = $conn->query("SELECT * FROM `admin` WHERE `admin_id` = '$_SESSION[admin_id]'") or die($conn->error);
					$f_admin_side = $q_admin_side->fetch_array();
					echo "<center>".$f_admin_side['name']."</center>";
				?>
			</a>
		</li>
		<li><a href = "home.php"><i class = "glyphicon glyphicon-home"></i> Home</a></li>
		<li><a href = "admin.php"><i class = "glyphicon glyphicon-user"></i> Administrator</a></li>
        <li><a href = "students_details.php"><i class = "glyphicon glyphicon-user"></i> Students Acc</a></li>
		<li><a href = "company_details.php"><i class = "glyphicon glyphicon-user"></i> Company Acc</a></li>
		<li><a href = "internship_student.php"><i class = "glyphicon glyphicon-user"></i>Internship students</a></li>
		<li><a href = "activity.php"><i class = "glyphicon glyphicon-calendar"></i>Upload internships</a></li>
		<li><a href = "student.php"><i class = "glyphicon glyphicon-ruble"></i> Students partculars</a></li>
		<li><a href = "logout.php"><i class = "glyphicon glyphicon-log-out"></i> Logout</a></li>
	</ul>
</div>