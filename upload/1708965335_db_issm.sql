...
--
DELIMITER //
CREATE TRIGGER delete_activity_when_quantity_zero
BEFORE UPDATE ON activity
FOR EACH ROW
BEGIN
    IF NEW.quantity = 0 THEN
        DELETE FROM activity WHERE activity_id = NEW.activity_id;
    END IF;
END;
//
DELIMITER ;

-- Add an index on the `end` column for efficient querying
ALTER TABLE activity ADD INDEX end_index (`end`);

-- Create a script to delete expired records
DELIMITER $$
CREATE PROCEDURE delete_expired_activities()
BEGIN
    DELETE FROM activity WHERE STR_TO_DATE(`end`, '%Y-%m-%d') < CURRENT_TIMESTAMP();
END$$
DELIMITER ;
...
CREATE EVENT IF NOT EXISTS delete_expired_events
ON SCHEDULE EVERY 1 HOUR
DO
    CALL delete_expired_activities();
...
(2,'Test Activity', 'Test Description', 'Test Company', 10, '2024-01-01', '2024-02-20', 'February', '2024'),
(3, 'developer ', 'candidate must have background of front end and know at least one framework per language ','company2', 2, '011217', '012318', 'Oct', '2023');
...
INSERT INTO `company` (`company_id`, `username`, `password`, `URL`, `name`,`email`,`desc`) VALUES
(1, 'company1', 'company1','https://www.google.com', 'Company1','company@gmail.com','Software Company');
